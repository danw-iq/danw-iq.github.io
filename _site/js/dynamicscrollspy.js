...
(function($) {
  ...
  $.fn.DynamicScrollspy = function(opts) {
    ...
    function renderTree() {
      ...
      //var ul = $('<ul class="nav child ' + self.options.ulClassNames + '"></ul>');
      var ul = $('<ul id="side-menu" class="nav ' + self.options.ulClassNames + '"></ul>');
      self.append(ul);
      //then iterate three tree
      $.each(self.tree, function(k) {
        var c = self.tree[k];
        var li = '<li id="dsli' + k + '"><a href="#' + k + '">' + c.dstext + '</a></li>';
       // var li = '<li><a href="#' + k + '">' + c.dstext + '</a></li>';
        ul.append(li);
    ...
    function itRenderTree(what) {
      ...
      for (var k in what) {
        ...
        if (k == 'dstext' || k == 'jqel') continue;
        var c = what[k];
        ul.append('<li id="dsli' + k + '"><a title="' + c.dstext + '" href="#' + k + '">' + c.dstext + '</a></li>');
        itRenderTree(what[k]);
      }

      //wrap a span around first child (anchor tag)

      $("ul.nav-second-level").each(function() {
          $(this).prev().replaceWith('<a href="#">' + $(this).prev().text() + '<span class="fa arrow"></span></a>');
      });
