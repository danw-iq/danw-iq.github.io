...
It is your organization's responsibility to ensure your Identity Provider is highly available. If you are unsure what that requires or if you anticipate problems, please let contact {{contact_support}} and we can discuss ways to mitigate these.
...
When you are ready to test the integration, contact {{contact_support}} to request the 3PA testing utility.
...
To maintain this mapping, you will need to update {{iqmetrixhub}} or {{rq}} when new roles are added or existing roles are updated.

In addition, you will need to set and maintain a mapping using the ClientEntityId property on {{CompanyTreeNode}} resources. This must be done using the Company Tree API.
